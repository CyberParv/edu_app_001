...
export default function EnrollmentDetailScreen() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.back();
    } catch { showToast('Unenroll failed', 'error'); }
      ...
      <ScrollView className="flex-1 bg-gray-50 dark:bg-gray-900">
        ...
        <View className="p-4 -mt-6">
          ...
          <Card mode="elevated" className="mb-4">
            ...
            <Card.Content>
              <View className="flex-row justify-between py-2 border-b border-gray-100 dark:border-gray-800">
                ...
                <Text className="font-medium text-gray-900 dark:text-white">{course.progress}%</Text>
              </View>
            </Card.Content>
          </Card>
          ...
          <View className="flex-row gap-3 mb-8">
            ...
            <Button mode="outlined" icon="delete" textColor="red" onPress={() => setDeleteVisible(true)} className="flex-1">Unenroll</Button>
          </View>
        </View>
      </ScrollView>
      <ConfirmDialog visible={deleteVisible} title="Unenroll?" message="This cannot be undone." onConfirm={handleDelete} onDismiss={() => setDeleteVisible(false)} loading={deleting} />
    </>
  );
}
